...
public class ChunkProviderFrozenHearth implements IChunkProvider{
    private Random frozenRNG;
	...
	for (int k = 0; k < 16; ++k)
	{
		...
		//biomegenbase.genTerrainBlocks(this.worldObj, this.frozenRNG, p_147418_3_, meta, p_147418_1_ * 16 + k, p_147418_2_ * 16 + l, this.porphyryExclusiveNoise[l + k * 16]);
		float f = biomegenbase.getFloatTemperature(k, 1, l);
		boolean flag = this.lateriteGrassNoise[k + l * 16] + this.frozenRNG.nextDouble() * 0.2D > 0.0D;
		boolean flag1 = this.porphyryNoise[k + l * 16] + this.frozenRNG.nextDouble() * 0.2D > 0.0D;
		int i1 = (int)(this.porphyryExclusiveNoise[k + l * 16] / 3.0D + 3.0D + this.frozenRNG.nextDouble() * 0.25D);
		int j1 = -1;
		...
		for (int k1 = 127; k1 >= 0; --k1)
		{
		    ...
		    if (k1 < 127 - this.frozenRNG.nextInt(5) && k1 > 0 + this.frozenRNG.nextInt(5))
		    {
    ...
    {
	this.frozenRNG.setSeed((long)p_73154_1_ * 341873128712L + (long)p_73154_2_ * 132897987541L);
	...
	MinecraftForge.EVENT_BUS.post(new PopulateChunkEvent.Pre(p_73153_1_, worldObj, frozenRNG, p_73153_2_, p_73153_3_, false));

	int k = p_73153_2_ * 16;
	int l = p_73153_3_ * 16;
	BiomeGenBase biomegenbase = this.worldObj.getBiomeGenForCoords(k + 16, l + 16);
	//this.genNetherBridge.generateStructuresInChunk(this.worldObj, this.frozenRNG, par2, par3);
	int i1;
	int j1;
	int k1;
	int l1 = 0;
	...
	biomegenbase.decorate(this.worldObj, this.frozenRNG, k, l);
	//boolean doGen = TerrainGen.populate(p_73153_1_, worldObj, frozenRNG, p_73153_2_, p_73153_3_, false, NETHER_LAVA);

	MinecraftForge.EVENT_BUS.post(new DecorateBiomeEvent.Pre(worldObj, frozenRNG, k, l));

	i1 = this.frozenRNG.nextInt(this.frozenRNG.nextInt(10) + 1) + 1;
	int i2;
	...
	MinecraftForge.EVENT_BUS.post(new DecorateBiomeEvent.Post(worldObj, frozenRNG, k, l));
	MinecraftForge.EVENT_BUS.post(new PopulateChunkEvent.Post(p_73153_1_, worldObj, frozenRNG, p_73153_2_, p_73153_3_, false));
